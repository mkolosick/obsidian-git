...
export default class ObsidianGit extends Plugin {
    ...
    async loadPlugin() {
        ...
        this.addCommand({
			id: "stage-file-and-commit",
			name: "Stage current file and commit",
            checkCallback: (checking) => {
                const file = this.app.workspace.getActiveFile();
                if (checking) {
                    return file !== null;
                } else {
                    this.promiseQueue.addTask(() => this.commitFile(file!));
                }
            }
		});
    ...
    async commitFile(file: TFile): Promise<boolean> {
		await this.stageFile(file);
		await this.commit(false, false, true)
        return true;
    }
